...
function printSchedule(scheduleArr) {
    ...
    // /*Time*/
        ...
        if (scheduleArr[i].isPast() && scheduleArr[i].getCompletedStatus() == 0) {
            itemTime.style.color = "red";
        }
        if (scheduleArr[i].getCompletedStatus() == 1 || (scheduleArr[i].isPast() && scheduleArr[i].getTaskName() == "---- BREAK TIME ----")) { 
            console.log("I come here");
            itemTime.style.color = "#AAAAAA";
        }
        if (scheduleArr[i].getStartTimeInMs() <= Date.now() && Date.now() <= scheduleArr[i].getEndTimeInMs()) {
            itemTime.style.backgroundColor = "#d6f4ff";
            itemTime.style.borderRadius = "4px";
            itemTime.style.border = "black";
            itemTime.style.marginLeft= "15px";
            itemTime.style.height= "40px";
            itemTime.style.width= "350px";
        }
    ...
    //     /*itemName*/
        ...
        if (scheduleArr[i].getCompletedStatus() == 1 || (scheduleArr[i].isPast() && scheduleArr[i].getTaskName() == "---- BREAK TIME ----")) {
            itemName.style.color = "#AAAAAA";
        }
        if (scheduleArr[i].getStartTimeInMs() <= Date.now() && Date.now() <= scheduleArr[i].getEndTimeInMs()) {
            itemName.style.backgroundColor = "#d6f4ff";
            itemName.style.borderRadius = "4px";
            itemName.style.border = "black";
            itemName.style.marginTop= "6px";
            itemName.style.marginLeft= "-100px";
            itemName.style.height= "42px";
            itemName.style.width= "437px";
        }
...
function tempFixed(object) { //only reschedule, edit and delete
    ...
    localStorage.setItem("taskCat", object.getTaskCat());
    ...
    newEdit.id = "edit";
    newEdit.innerHTML = '<button type="submit" class="btn" style="background-color=#ECEDEA;border-radius=5px;border-width=2px;"><i class="fa fa-pencil-square-o fa-2x" aria-hidden="true"></i></button>';
    ...
    taskCat.type = "hidden";
    taskCat.value = object.getTaskCat();
